...
> 状态: **已确认** | 版本: v0.7.0 | 创建: 2026-02-15 | 更新: 2026-02-17
...
> **Odoo 开发注意事项**（试跑经验总结）：
> - Sprint 0 安装标准模块时，先完成所有配置项（币种、审批、协议等），再开发自定义模块
> - `_inherit` 扩展标准模型时，注意 `_approval_allowed` 等核心方法的覆盖逻辑
> - `@api.constrains` 只在装饰器中列出的字段被 `create/write` 显式传递时触发——若需确保约束始终生效，需把必传字段（如 `name`）加入列表
> - 测试中跨模块字段（如 `sale` 模块的 `sale_line_warn`）可能导致 `NOT NULL` 约束冲突，需在 `setUpClass` 中设置数据库默认值
> - Odoo 安全组的 `implied_ids`（组继承）会影响权限边界，设计 RBAC 时必须画出组继承关系图

**主导方法论**：Agile/Scrum（Sprint 迭代）+ TDD + BDD

**进入条件（Entry）**：
- Phase 1 退出条件全部满足（分析产出物全部确认）
- Gap-Fit 分析至少完成 1 个核心业务域（方法验证）
- Sprint 规划已基于 Gap-Fit 结果排定
...
- **自定义模块**测试覆盖率 ≥ 80%（标准 Odoo 模块不计入覆盖率，仅测试配置正确性）
- 核心页面响应时间 < 2s
...
> **试跑经验**：Odoo 项目中"测试覆盖率 ≥ 80%"应限定为自定义模块（`custom_addons/`）。标准模块由 Odoo 社区维护，项目层只需验证配置正确性（如模块安装成功、功能启用后可用）。
...
> **试跑经验**：Phase 3 试跑验证了完整的测试→部署→培训闭环。即使只有部分模块（Sprint 0+1），仍可有效执行 Phase 3 方法。关键发现：
> - 回滚演练非常快速（Docker 环境 < 10 秒），应作为必做项而非可选项
> - E2E 测试发现了 2 个真实 Bug（P1 审批逻辑、P2 约束触发），证明测试价值
> - 安全审计发现 3 个 P2 配置差异（dev vs prod），`docker-compose.prod.yml` 分离是关键
> - 1 人项目 UAT 可由开发者按 BDD 场景自测，但需严格按步骤走，避免"开发者直觉跳过"
...
| v0.7.0 | 2026-02-17 | 基于 Phase 1~3 试跑反馈：修复 Phase 2 进入条件（移除代码要求）、精化 Phase 2→3 门禁（区分标准模块与自定义模块覆盖率）、补充 Odoo 特有注意事项 |
